...
app = FastAPI(title="Golbert PRO v5.3")
...
def init_db():
    ...
    c.execute("CREATE TABLE IF NOT EXISTS vpn_users (username TEXT PRIMARY KEY, node_ip TEXT, exp_date TEXT)")
...
class VPNCreate(BaseModel): username: str; password: str; days: int; node_ip: str
class Pago(BaseModel): plan: str; metodo: str
...
# --- CONFIG PAGOS - TUYO ---
MERCADOPAGO_TOKEN = os.getenv("MP_TOKEN", "TEST-XXXXXXXX")
YAPE_QR_URL = "https://i.imgur.com/xxxxx.png" # sube tu QR de Yape
YAPE_NUMERO = "960084993" # CORREGIDO
SOPORTE_NUM = "51960084993"
...
@app.get("/stats")
def stats(): return {"cpu": psutil.cpu_percent(), "ram": psutil.virtual_memory().percent, "ram_detail": psutil.virtual_memory()._asdict()}

# --- VPN ---
@app.post("/vpn/create")
def create_vpn(v: VPNCreate, user=Depends(get_user)):
    # Llama al nodo para crear usuario SSH real
    try:
        r = requests.post(f"http://{v.node_ip}:8000/create", json={"username": v.username, "password": v.password, "days": v.days}, timeout=10)
        if r.status_code != 200:
            raise HTTPException(400, f"Nodo error: {r.text}")
    except Exception
¡Gracias por confiar en Golbert! 🚀`;" # Tu número
